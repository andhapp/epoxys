...
class PasteBinApi
	...
	class << self # makes a singleton class
		...
		# The first conditional deals with paste_create_new and the second one with paste_id, paste_url 
		def method_missing(name, *args, &block)
			if name.to_s =~ /^(paste_)(.*)$/ and !respond_to?(name)
				if $2.eql?("create_new")
					...
					@post_detail[1]["#{$2.upcase}"] # the postdetails object is supplied back like this [true, ["URL" => "", "ID" => ""]]
				end
			end
